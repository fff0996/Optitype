...
if __name__ == '__main__':
    ...
    ALLELE_HDF = os.path.join(this_dir, 'data/alleles_korean.h5')
    MAPPING_REF = {'gen': os.path.join(this_dir, 'data/hla_reference_dna.korean.fasta'),
                   'nuc': os.path.join(this_dir, 'data/hla_reference_rna.korean.fasta')}
    ...
    out_exp = os.path.join(out_dir,("%s_expr_result.tsv" % prefix))
    out_table = os.path.join(out_dir,("%s_table_result.tsv" % prefix))
    ...
    coverage_mat,expression_df = ht.calculate_coverage(plot_variables, features, hlatype, features_used)

    ht.plot_coverage(out_plot, coverage_mat, table, features, features_used)
    expression_df.to_csv(out_exp,sep="\t",index=False)




    allele_map=pd.DataFrame({'Original': result.values.flatten(),'HLA_4digit': result_4digit.values.flatten()
    }).drop_duplicates().dropna()
    allele_map.to_csv(out_table, sep='\t', index=False)
