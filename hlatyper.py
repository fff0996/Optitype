...
def calculate_coverage(alignment, features, alleles_to_plot, features_used):
    ...
    for allele in alleles_to_plot:
        ...
        allele_lengths[allele] = allele_length
        # Dimensions:
        ...
        for pos, read_details, hit_counts, pairing_info in to_process:
            ...
            for i_pos, i_read_length, i_mismatches, i_hitcount, i_pairing in zip(
                ...
                if i_mismatches == 0 and i_pairing ==1 and i_hitcount ==1:
                    read_count +=1

        coverage_matrices.append((allele, coverage))
        allele_read_counts[allele]=read_count
    rpk = {allele: count/ (allele_lengths[allele]/1000) if allele_lengths[allele] > 0 else 0
            for allele, count in allele_read_counts.items()}
    total_rpk = sum(rpk.values())
    tpm = {allele: (rpk[allele] / total_rpk *1e6) if total_rpk > 0 else 0 for allele in alleles_to_plot}
    #total_count = sum(allele_read_counts.values())
    #cpm = {allele: (count / total_count * 1e6) if total_count > 0 else 0 for allele, count in allele_read_counts.items()}
    #with open("perfect_paired_unique_read_count.tsv","w") as f_out:
     #   f_out.write("Allele\tPerfect_paired_unique_read_count\tLength\tTPM\tLog_TPM\n")
    data =[]
    for allele in alleles_to_plot:
            #pretty_name = get_types(allele) if allele is not None else allele
            count = allele_read_counts.get(allele, 0)
            length = allele_lengths.get(allele, 0)
            tpm_value = tpm.get(allele, 0)
            log_tpm = np.log2(tpm_value + 1)

            data.append({"Allele": allele,"Perfect_paired_unique_read_count":count,"Length":length,"TPM":round(tpm_value,2),"Log_TPM":round(log_tpm,2)})
            #f_out.write(f"{allele}\t{count}\t{length}\t{tpm_value:.2f}\t{log_tpm:.2f}\n")
    df = pd.DataFrame(data)
    return coverage_matrices,df
